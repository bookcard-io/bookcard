...
def test_get_library_stats(temp_repo: CalibreBookRepository) -> None:
    ...
    with temp_repo._get_session() as session:
        # Create test data
        ...
        session.add(Data(book=1, format="EPUB", uncompressed_size=1000, name="book1"))
        session.add(Data(book=2, format="PDF", uncompressed_size=2000, name="book2"))
        session.commit()

    stats = temp_repo.get_library_stats()
            ...
            try:
                ...
                with patch.object(
                    ...
                    book_id = repo.add_book(
                        test_file, "epub", library_path=library_path
                    )
        ...
        try:
            ...
            try:
                with repo._get_session() as session:
                    # Add data records
                    data1 = Data(
                        book=1, format="EPUB", uncompressed_size=1000, name="Title"
                    )
        ...
        try:
            ...
            try:
                with repo._get_session() as session:
                    ...
                    try:
                        filesystem_paths, _result_book_dir = (
                            repo._collect_filesystem_paths(
                                session, 1, "Author/Title", library_path
                            )
                        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            try:
                with repo._get_session() as session:
                    book = Book(
                        id=1, title="Test Book", uuid="test-uuid", path="Author/Title"
                    )
                    session.add(book)
                    session.flush()
                ...
                repo.delete_book(
                    1, delete_files_from_drive=True, library_path=library_path
                )
...
def test_get_library_stats_total_content_size_none(
    ...
    temp_repo.get_library_stats = types.MethodType(  # type: ignore[assignment]
        test_get_library_stats_with_none_check, temp_repo
    )
        ...
        try:
            ...
            try:
                ...
                with patch.object(
                    ...
                    cover_path = (
                        library_path / "Test Author" / "Test Book" / "cover.jpg"
                    )
                    assert cover_path.exists()
            finally:
                repo.dispose()
...
def test_list_books_with_full_details(temp_repo: CalibreBookRepository) -> None:
    ...
    with temp_repo._get_session() as session:
        ...
        with patch.object(
            temp_repo, "_enrich_books_with_full_details", return_value=[]
        ) as mock_enrich:
            temp_repo.list_books(limit=10, offset=0, full=True)

            mock_enrich.assert_called_once()
            assert isinstance(mock_enrich.call_args[0][1], list)
            assert all(
                isinstance(b, BookWithRelations) for b in mock_enrich.call_args[0][1]
            )
...
def test_list_books_with_filters_full_details(temp_repo: CalibreBookRepository) -> None:
    ...
    with temp_repo._get_session() as session:
        ...
        with patch.object(
            ...
            temp_repo.list_books_with_filters(limit=10, offset=0, full=True)
...
def test_fetch_tags_map(temp_repo: CalibreBookRepository) -> None:
    ...
    with temp_repo._get_session() as session:
        ...
        tags_map = temp_repo._fetch_tags_map(session, [1])
...
def test_fetch_identifiers_map(temp_repo: CalibreBookRepository) -> None:
    ...
    with temp_repo._get_session() as session:
        ...
        identifiers_map = temp_repo._fetch_identifiers_map(session, [1])
...
def test_fetch_descriptions_map(temp_repo: CalibreBookRepository) -> None:
    ...
    with temp_repo._get_session() as session:
        ...
        descriptions_map = temp_repo._fetch_descriptions_map(session, [1])
...
def test_fetch_publishers_map(temp_repo: CalibreBookRepository) -> None:
    ...
    with temp_repo._get_session() as session:
        ...
        publishers_map = temp_repo._fetch_publishers_map(session, [1])
...
def test_fetch_languages_map(temp_repo: CalibreBookRepository) -> None:
    ...
    with temp_repo._get_session() as session:
        ...
        languages_map = temp_repo._fetch_languages_map(session, [1])
...
def test_fetch_ratings_map(temp_repo: CalibreBookRepository) -> None:
    ...
    with temp_repo._get_session() as session:
        ...
        ratings_map = temp_repo._fetch_ratings_map(session, [1])
...
def test_fetch_formats_map(temp_repo: CalibreBookRepository) -> None:
    ...
    with temp_repo._get_session() as session:
        ...
        formats_map = temp_repo._fetch_formats_map(session, [1])

        assert 1 in formats_map
        assert len(formats_map[1]) == 2
        assert {"format": "EPUB", "size": 1024, "name": "test.epub"} in formats_map[1]
...
def test_fetch_series_ids_map(temp_repo: CalibreBookRepository) -> None:
    ...
    with temp_repo._get_session() as session:
        ...
        series_ids_map = temp_repo._fetch_series_ids_map(session, [1])
...
def test_enrich_books_with_full_details_empty_list(
    ...
    with temp_repo._get_session() as session:
        result = temp_repo._enrich_books_with_full_details(session, [])
...
def test_enrich_books_with_full_details_none_ids(
    ...
    with temp_repo._get_session() as session:
        ...
        result = temp_repo._enrich_books_with_full_details(session, [book_with_rels])
