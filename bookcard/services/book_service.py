...
from bookcard.repositories import (
...
class BookService:
    ...
    def delete_book(
        ...
        Deletes both Calibre database associations and Bookcard database associations.
    ...
    def _delete_bookcard_associations(self, book_id: int) -> None:
        ...
        Notes
        -----
        This method requires a valid Bookcard database session.
        If session is None, this method does nothing (no-op).
        """
        if self._session is None:
            logger.warning(
                "Cannot delete Bookcard associations for book_id=%d: no session available",
                book_id,
            )
            return
        ...
        try:
            ...
            logger.info(
                "Successfully deleted all Bookcard database associations for book_id=%d",
                book_id,
            )
    ...
    def _delete_associations_by_model(
        ...
        from _delete_bookcard_associations after session validation.
        """
        if self._session is None:
            return
